...
"""
This file provides configuration to build U-NET with VoxelDCL 
for semantic segmentation. (Speically for ADNI dataset.)
...
def configure():
    ...
    flags.DEFINE_integer('test_step', 163000, 'Test or predict model at this step')
    # Debug
    ...
    flags.DEFINE_string(
        'conv_name', 'conv',
        'Use which conv op in decoder: currently only convolution is avaliable. ')
    flags.DEFINE_string(
        'deconv_name', 'ivoxel_dcl',
...
def main(index):
    ...
    parser.add_argument('--option', dest='option', type=str, default='train',
                        help='action: train, test, or predict')
    args = parser.parse_args()
    sess = tf.Session()
    conf = configure()
    model = VoxelDCN(sess,conf,'predict')
	...
	#model = DenseTransformerNetwo:rk(tf.Session(), configure())
	#valid(model,16,4)
        predict(1,20,'valid')
